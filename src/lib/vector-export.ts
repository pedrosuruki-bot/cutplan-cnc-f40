...
function escapeXml(v: string): string { return v.replace(/[&<>\"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]!); }
...
function safe(name: string): string { return name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-|-$/g, "").toLowerCase() || "cutplan"; }

export function exportLayoutSvg(project: Project, result: OptimizationResult): void {
  const gapY = 180;
  const totalH = result.layouts.reduce((sum, l) => sum + l.width, 0) + Math.max(0, result.layouts.length - 1) * gapY;
  let yOffset = 0;
  const groups = result.layouts.map((l, i) => {
    const currentY = yOffset;
    yOffset += l.width + gapY;
    const parts = l.placements.map((p) => `<g><rect x="${p.x}" y="${currentY + p.y}" width="${p.w}" height="${p.h}" fill="${colorForPart(p.partId)}" fill-opacity="0.35" stroke="#222" stroke-width="2"/><text x="${p.x + p.w / 2}" y="${currentY + p.y + p.h / 2}" text-anchor="middle" dominant-baseline="middle" font-size="28">${escapeXml(p.partId)}</text></g>`).join("");
  ...
  download(new Blob([content], { type: "image/svg+xml;charset=utf-8" }), `${safe(project.name)}-planos.svg`);
}
...
export function exportLayoutDxf(project: Project, result: OptimizationResult): void {
  ...
  for (const l of result.layouts) {
    out += polyline([[0, 0], [l.length, 0], [l.length, l.width], [0, l.width], [0, 0]], `SHEET_${l.index}`);
    for (const p of l.placements) out += polyline([[p.x, p.y], [p.x + p.w, p.y], [p.x + p.w, p.y + p.h], [p.x, p.y + p.h], [p.x, p.y]], `PART_${p.partId}_${p.instance}`);
  }
  out += "0\nENDSEC\n0\nEOF\n";
  download(new Blob([out], { type: "application/dxf" }), `${safe(project.name)}-planos.dxf`);
}

export function exportLayoutSvgSingle(project: Project, layout: OptimizationResult["layouts"][number]): void {

...
export interface AltendorfProgress {
  progress: number;
  placedCount: number;
  placedArea: number;
  strips: Strip[];
...
function orderInstances(instances: Instance[], sheet: Sheet, params: CutParameters, variant: number): Instance[] {
  ...
  list.sort((a, b) => {
    const aa = a.part.length * a.part.width;
    const ab = b.part.length * b.part.width;
    const la = Math.max(a.part.length, a.part.width);
    const lb = Math.max(b.part.length, b.part.width);
    const sa = Math.min(a.part.length, a.part.width);
    const sb = Math.min(b.part.length, b.part.width);
    const da = difficulty(a, sheet, params);
    const db = difficulty(b, sheet, params);
    switch (variant % 10) {
...
function evaluateSequence(
  ...
  for (let index = 0; index < sequence.length; index++) {
    if (deadline && (index & 15) === 0 && Date.now() >= deadline) {
      remaining.push(...sequence.slice(index));
      break;
    }
    const inst = sequence[index]!;
    const opts = candidates(inst, params).filter(
      (o) => o.w <= usableL + EPS && o.h <= usableW + EPS,
    );
    if (!opts.length) {
      remaining.push(inst);
      continue;
  ...
  const placedArea = strips.reduce(
    (sum, s) => sum + s.items.reduce((v, i) => v + i.w * i.h, 0),
    0,
  );
...
function toPlacements(best: Evaluated, sheet: Sheet, params: CutParameters): Placement[] {
  const margin = Math.max(0, params.margin);
  return best.strips.flatMap((strip) =>
...
function searchBest(
  ...
  const emit = (candidate: Evaluated) => {
    ...
    const now = Date.now();
    if (now - lastEmit < 250 && now < deadline && candidate !== best) return;
    lastEmit = now;
    onProgress({
      progress: Math.min(99, Math.max(0, Math.round(((now - started) / totalWindow) * 100))),
      placedCount: candidate.placedCount,
      placedArea: candidate.placedArea,
      strips: candidate.strips,
    });
  };
  ...
  if (!best || !bestSequence) {
    ...
    best = evaluateSequence(fallback, sheet, params, Date.now() + 1);
  }
...
export function packForAltendorfF40(
  ...
  const placements = toPlacements(best, sheet, params);
  const cuts: CutStep[] = [];
  let order = 1;
  ...
  for (let si = 0; si < best.strips.length; si++) {
    ...
    if (si < best.strips.length - 1) {
      cuts.push({
        ...
        type: "horizontal",
        position: r(y1),
        from: r(Math.max(0, params.margin)),
        to: r(sheet.length - Math.max(0, params.margin)),
    ...
    for (const item of strip.items.slice().sort((a, b) => a.x - b.x)) {
      ...
      if (right < sheet.length - Math.max(0, params.margin) - EPS) {
        cuts.push({
          order: order++,
          phase: "crosscut",
          type: "vertical",
  ...
  for (const p of placements) {
    if (
      p.x < params.margin - 1e-6 ||
      p.y < params.margin - 1e-6 ||
      p.x + p.w > sheet.length - params.margin + 1e-6 ||
      p.y + p.h > sheet.width - params.margin + 1e-6
    ) {

...
export interface PlacedRect extends Rect { rotated: boolean }
interface Candidate { rect: Rect; rotated: boolean; score1: number; score2: number }
...
export class MaxRectsBin {
  ...
  private free: Rect[];
  private used: Rect[] = [];
  constructor(width: number, height: number) { this.width = width; this.height = height; this.free = [{ x: 0, y: 0, w: width, h: height }]; }
  get usedRects(): Rect[] { return this.used.slice(); }
  get freeRects(): Rect[] { return this.free.slice(); }
  insert(w: number, h: number, allowRotate: boolean, heuristic: FitHeuristic, gap: number): PlacedRect | null {
    const best = this.findPosition(w, h, allowRotate, heuristic);
    if (!best) return null;
    this.occupy(best.rect, gap);
    return { ...best.rect, rotated: best.rotated };
  }
  private findPosition(w: number, h: number, allowRotate: boolean, heuristic: FitHeuristic): Candidate | null {
    let best: Candidate | null = null;
    for (const fr of this.free) {
      const options = [{ w, h, rotated: false }, ...(allowRotate && w !== h ? [{ w: h, h: w, rotated: true }] : [])];
      for (const opt of options) {
        ...
        const [score1, score2] = heuristic === "best-area"
          ...
          : heuristic === "best-short-side"
            ...
            : [fr.y + opt.h, fr.x];
        if (!best || score1 < best.score1 - 1e-9 || (Math.abs(score1 - best.score1) < 1e-9 && score2 < best.score2)) best = { rect, rotated: opt.rotated, score1, score2 };
      }
    }
    return best;
  }
...
export function pruneContained(rects: Rect[]): Rect[] {
  return rects.filter((a, i) => !rects.some((b, j) => i !== j && contained(a, b) && !(j > i && contained(b, a))));
}
export function deriveFreeRects(width: number, height: number, placements: Rect[], gap: number): Rect[] {
  let free: Rect[] = [{ x: 0, y: 0, w: width, h: height }];
  for (const rect of placements) {
    const buffer = Math.max(0, gap) / 2;
    const blocked = { x: Math.max(0, rect.x - buffer), y: Math.max(0, rect.y - buffer), w: Math.min(width, rect.x + rect.w + buffer) - Math.max(0, rect.x - buffer), h: Math.min(height, rect.y + rect.h + buffer) - Math.max(0, rect.y - buffer) };
    const next: Rect[] = [];

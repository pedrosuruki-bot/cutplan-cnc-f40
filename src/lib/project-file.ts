...
export interface ProjectBackup { version: 1; exportedAt: string; project: Project }
...
export async function importProjectJson(file: File): Promise<Project> {
  ...
  const candidate = raw && typeof raw === "object" && "project" in raw ? (raw as { project?: unknown }).project : raw;
  if (!candidate || typeof candidate !== "object") throw new Error("Ficheiro de projeto inválido.");
  const p = candidate as Partial<Project>;
  if (typeof p.name !== "string" || !Array.isArray(p.sheets) || !Array.isArray(p.parts)) throw new Error("O ficheiro não contém um projeto CutPlan válido.");
  ...
  return {
    ...
    parts: p.parts,
    parameters,
    offcutStock: Array.isArray(p.offcutStock) ? p.offcutStock.filter((o) => o && typeof o === "object") as Project["offcutStock"] : [],
  } as Project;
}

function safeName(value: string): string { return value.trim().replace(/[^\p{L}\p{N}_-]+/gu, "-").replace(/^-+|-+$/g, "").slice(0, 80); }

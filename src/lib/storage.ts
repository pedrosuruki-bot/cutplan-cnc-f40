...
function normalizeProject(project: Project): Project {
  ...
  return {
    ...project,
    parameters,
    offcutStock: Array.isArray(project.offcutStock) ? project.offcutStock : [],
  };
}
function readLocal(): RecordData | null {
  ...
  try {
    ...
    if (parsed.project && typeof parsed.project === "object")
      return { ...(parsed as RecordData), project: normalizeProject(parsed.project) };
...
function writeLocal(record: RecordData): void {
  try {
...
export const projectRepository = {
  ...
  async loadPersistent(): Promise<RecordData | null> {
    ...
    return new Promise((resolve) => {
      ...
      req.onsuccess = () => resolve((req.result as RecordData | undefined) ?? local);
      req.onerror = () => resolve(local);
    });
  },
  save(project: Project, result: OptimizationResult | null = null): void {
    ...
    void openDb().then((db) => {
      ...
      tx.objectStore(STORE).put({ id: "current", ...record });
    });
  },
  saveResult(project: Project, result: OptimizationResult | null): void {
    this.save(project, result);
  },
  clear(): void {
